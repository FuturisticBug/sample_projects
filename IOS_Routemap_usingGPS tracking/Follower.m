...
- (void)handleLocationUpdate:(CLLocation *)location {
    ...
    //    NSLog(@"speed is%f",location.speed);
    
    
    speed = location.speed;
    
    //speed issue fixed by santanu
   ...
   // NSLog(@"speed is%f",speed);
    ...
    if ([location distanceFromLocation:self.previousLocation] >0) {
   ...
   // NSLog(@"distance change is%f",[location distanceFromLocation:self.previousLocation]);
...
- (void)createMarkers {
    ...
    _strLocation=self.routeLocations[0];
    
    _endLocation=self.routeLocations[locationsCount-1];
...
- (CLLocationDistance)totalDistanceWithUnit:(DistanceUnit)distanceUnit {
  ...
  //  NSLog(@"distance is%f", [self convertedDistance:self.totalDistance withUnit:distanceUnit]);
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations {
    ...
    for (CLLocation *location in locations) {
        ...
        //NSLog(@"Location data longitude... %f",location.coordinate.longitude);
           ...
           _bounds = [_bounds includingCoordinate:CLLocationCoordinate2DMake(location.coordinate.latitude, location.coordinate.longitude)];
        
                [_path addCoordinate:CLLocationCoordinate2DMake(location.coordinate.latitude, location.coordinate.longitude)];
